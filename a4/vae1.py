...
def vae_loss_function(reconstructed_x, x, means, log_variances):
    ...
    reconstruction_loss = cross_entropy_loss(reconstructed_x, x)
    # KL loss
    KL_element = means.pow(2).add_(log_variances.exp()).mul_(-1).add_(1).add_(log_variances)
...
def train(model, optimizer, epoch):
    ...
    return avg_train_loss / len(train_loader), avg_train_reconstruction_loss / len(train_loader)
...
def test(model, epoch):
    ...
    return avg_test_loss / len(test_loader), avg_test_reconstruction_loss / len(test_loader)
...
for epoch in range(1, epochs + 1):
    avg_train_loss, avg_train_reconstruction_loss = train(vae_model, vae_optimizer, epoch)
    avg_test_loss, avg_test_reconstruction_loss = test(vae_model, epoch)
